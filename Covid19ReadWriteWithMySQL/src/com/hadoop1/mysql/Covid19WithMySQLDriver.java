...
public class Covid19WithMySQLDriver extends Configured implements Tool{
	...
	public int run(String[] arg0) throws Exception {
		...
		job.setReducerClass(Covid19WithMySQLReducer.class);
		
		// mapper => reducer key, value type regist
		job.setOutputKeyClass(Text.class);
		job.setOutputValueClass(Covid19MaxDateWritable.class);
		...
		DBInputFormat.setInput(
				job,
				Covid19EntityRecorder.class,
				"SELECT * FROM covid19 ORDER BY iso_code",	// search query
				"SELECT count(*) FROM covid19"				// row count
		);
